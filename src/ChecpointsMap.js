...
export function CheckpointsView({ documentData, setSignal }) {
  ...
  const disableMapAndSaveCheckpoint = async () => {
    ...
    if (newCheckpointName === "") {
      updatedCheckpointNames = [
        ...documentData.checkpointNames,
        `Checkpoint: ${documentData.checkpointNames.length + 1}`,
      ];
    } else {
      updatedCheckpointNames = [
        ...documentData.checkpointNames,
  ...
  const checkIfSafeToDeleteCheckpoint = async (index) => {
    ...
    const subtaskQuery = query(
      subtaskRef,
      where("taskId", "==", taskId),
      where("checkpoint", "==", documentData.checkpoints[index])
    );
            ...
            >
              ...
              {safeToDelete ? (
                ...
                <React.Fragment>
                  ...
                  >
                    Cannot delete checkpoint with active subtasks. Delete
                    existing subtasks and try again.
                  </DialogContentText>
                  <DialogActions>
                    <Button onClick={handleCloseConfirmDeleteDialog}>
                      Cancel
