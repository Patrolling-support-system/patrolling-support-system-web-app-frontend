...
import { Button, Dialog, DialogActions, DialogContent, DialogTitle, Divider, Grid, IconButton, InputLabel, List, ListItem, ListItemText, MenuItem, Paper, Select, TextField, Typography } from "@mui/material";
import { collection, documentId, getDocs, getFirestore, where } from "firebase/firestore";
        ...
        <Grid container spacing={2}>
          <Grid item xs={12} sm={12} container justifyContent={"center"}>
            <Typography variant="h5">
              Patrol group member reports:
            </Typography>
          </Grid>
          <Dialog open={openReportDetails} onClose={handleCloseReportDetails} fullWidth>
            <DialogTitle>
              Report details:
            </DialogTitle>
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={12}>
                  ...
                  {containsImages ? (
                    ...
                    <Typography>
                      No image files attached to report.
                    </Typography>
                  )}
                </Grid>
                <Grid item xs={12}>
                  <InputLabel>
                    Audio files attached to report:
                  </InputLabel>
                  {containsAudio ? (
                    <Select
                      value={selectedAudio}
                    ...
                    <Typography>
                      No audio files attached to report.
                    </Typography>
                  )}
                </Grid>
              </Grid>
                  ...
                  <List>
                    {reportList.map((item, index) => (
                      <React.Fragment key={index}>
                        <ListItem>
                          <ListItemText>
                            <Typography variant="h6" noWrap>Participant: {item.participantName}</Typography>
                            {item.images.length > 0 ? (
                              <Typography variant="body1" noWrap>Images: {item.images.length}</Typography>
                            ) : (
                              <Typography variant="body1" noWrap>No images</Typography>
                            )}
                            {item.recordings.length > 0 ? (
                              <Typography variant="body1" noWrap>Audio files: {item.recordings.length}</Typography>
                            ) : (
                              <Typography variant="body1" noWrap>No audio files</Typography>
                            )}
                          </ListItemText>
